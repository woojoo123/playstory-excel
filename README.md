...
이 과제에서는 복잡한 스키마 변경 이력을 관리할 필요가 없으므로 Flyway 같은
추가 도구를 도입하지 않았습니다. 실제 운영 환경에서 여러 버전의 스키마
변경을 추적하고 배포 순서를 관리해야 한다면 migration 도구를 검토합니다.
...
### 단일 worker와 Spring Scheduler

과제는 단일 백엔드 컨테이너와 제한된 리소스를 전제로 합니다. 별도 Redis,
...
이 방식은 조회한 데이터의 시작 위치가 뒤로 갈수록 DB가 앞부분을 반복해서
건너뛰어야 하는 offset pagination의 비용을 피합니다. 주문의 기본 키를
기준으로 다음 묶음을 찾는 keyset pagination 방식입니다.

완성되지 않은 파일이 다운로드되는 것을 막기 위해 먼저 `.xlsx.tmp` 파일에
작성합니다. 파일 생성이 끝나면 최종 `.xlsx` 경로로 이동하고 DB 상태를
`DONE`으로 변경합니다.
